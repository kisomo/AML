 ...
 struct //Structure for storing User information
{
 ...
 typedef struct //Structure for storing account information
{
	char ac_no[8];
 ...
 typedef struct //Structure for storing transaction information
{
...
int main()
{
	...
	do
	{
		...
		c = getchar(); //Variable c stores the key pressed by user
...
void staff()
{
	...
	do
	{
		...
		while (fscanf(fp, "%s %s %s\n", id, nam, pass) != EOF)
		{
			...
			strcpy(user.name, _strupr(user.name));
			strcpy(user.password, _strupr(user.password));
...
void add_ac()
{
	account ac;//structure variable ac created
	//char acn[8];
	char curr[35], ch;
	unsigned int i;
	float irate;
...
void uptodate()
{
	...
	if (fp != NULL)//performs every thing only when file exists i.e. only wen pointer is not null
	{
		...
		while (fscanf(fp, "%s %s %s %s %s %s %c %s %c %f %f %f\n", ac.ac_no, ac.fname, ac.lname, ac.dob, ac.address, ac.contact, &ac.sex, ac.u_date, &ac.ac_type, &ac.c_bal, &ac.interest, &ac.t_bal) != EOF)
		{
			...
			if (ac.ac_type == 'S' || ac.ac_type == 's') r = 8.00 / 365.00; else r = 4.00 / 365.00;
...
void deposit()
{
	...
	strcpy(acn, _strupr(acn)); //changing a/c no. to uppercase
	fp = fopen("ACCOUNT.DAT", "r");
	while (fscanf(fp, "%s %s %s %s %s %s %c %s %c %f %f %f\n", ac.ac_no, ac.fname, ac.lname, ac.dob, ac.address, ac.contact, &ac.sex, ac.u_date, &ac.ac_type, &ac.c_bal, &ac.interest, &ac.t_bal) != EOF)
...
void ac_info()
{
	...
	account ac, srh;//structure variable srh stores the matched data from data file
	float irate;
	char c, acn[8];
...
void transaction()
{
	...
	trans t, *selected;
	selected = (trans *)malloc(sizeof( trans)*nr);
...
void admin()
{
	...
	do
	{
		...
		strcpy(uname, _strupr(uname)); //Changes string uname into uppercase
		strcpy(pass, _strupr(pass));
	...
	do
	{
		...
		key = getchar();//Reads a character entered by user andstroes in char variable key
...
void view_log()
{
	...
	do {
		...
		case 1:
			...
			for (i = 0; i<l; i++)
			{
				strcpy(log[i].name, _strupr(log[i].name));
